...
!------------------------------------------------------------------
! Loop using the 2d grouping method to solve rate equations with 
! high density sink strenght corrections
!------------------------------------------------------------------  
  ...
  do while (t <= tmax)
   ...
   !& istate, iopt, rwork, lrw, iwork, liw, jac, mf)
    ...
    if (tstage.ge.prevt+1 .and. t .lt. tmax) then
      ...
      call clusterss(y,clss_V,clss_i,clss_He) !print sink strength values
      call printintvalues(t,clss_V,clss_i,clss_He) !print sink strength values
      call tolchangetest(y)  !Change the tolerance 
